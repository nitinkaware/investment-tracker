    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg mb-4">
        ...
        <div v-if="isVisible">
            <dl>
				...
				<div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    <dt class="text-sm leading-5 font-medium text-gray-500">Maturity Amount</dt>
                    <dd
                        class="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2"
                    >{{ maturityValue }}</dd>
                </div>
...
export default {
    ...
    computed: {
        ...
        interestEarnedUntilNow() {
            return `₹ ${this.toIndiaFormat(
                this.investment.interestEarnedUntilNow
            )}`;
        },
        currentInvestmentValue() {
            return `₹ ${this.toIndiaFormat(
                this.investment.currentInvestmentValue
            )}`;
        },
        monthlyInstallment() {
        ...
        maturityValue() {
            return `₹ ${this.toIndiaFormat(this.investment.maturityValue)} (${this.investment.investedTillRedable})`;
        },
    },
};
</script>
