    ...
    <div v-if="isVisible">
      <dl>
        ...
        <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
          ...
          <dd class="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2">{{ totalInterestEarned }}</dd>
        </div>
        <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
          <dt class="text-sm leading-5 font-medium text-gray-500">Final Investment Value</dt>
          <dd
            class="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2"
          >{{ finalInvestmentValue }}</dd>
        </div>
        <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
          <dt class="text-sm leading-5 font-medium text-gray-500">Started on</dt>
          <dd class="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2">{{ investment.investedAt }}</dd>
        </div>
        <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
          <dt class="text-sm leading-5 font-medium text-gray-500">Percentage Increase</dt>
          <dd class="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2">{{ investment.percentageIncreased }} %</dd>
        </div>
      </dl>
    </div>
  </div>
...
export default {
  ...
  computed: {
    withoutInterestInvestmentValue() {
        return `₹ ${this.toIndiaFormat(this.investment.withoutInterestInvestmentValue)}`
    },
    totalInterestEarned() {
        return `₹ ${this.toIndiaFormat(this.investment.totalInterestEarned)}`
    },
    finalInvestmentValue() {
        return `₹ ${this.toIndiaFormat(this.investment.finalInvestmentValue)}`
    },
  },
};
</script>
